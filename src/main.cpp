...
#if defined(NDEBUG)
# error "Liazoncoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x98cddc7ca18eb8342d6e6d082a2713d6ba598eedbb97bcbbf00f6efe7eabd0ed");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Liazoncoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Liazoncoin Signed Message:\n";
...
{
    // Liazoncoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1840000); // Liazoncoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 60 * 60; // Liazoncoin: 1 hour
static const int64 nTargetSpacing = 2 * 60; // Liazoncoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Liazoncoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Liazoncoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x98cddc7ca18eb8342d6e6d082a2713d6ba598eedbb97bcbbf00f6efe7eabd0ed");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "On Wed July 1 We created this ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1533133312;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1392146;
        ...
        assert(block.hashMerkleRoot == uint256("0x7af9b707fcc6631a7c3fcb9ac8681650055c331f7d1b8cebd6047f040813bf31"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Liazoncoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LiazoncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LiazoncoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LiazoncoinMiner(CWallet *pwallet)
{
    printf("LiazoncoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("liazoncoin-miner");
    ...
    try { loop {
        ...
        printf("Running LiazoncoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LiazoncoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LiazoncoinMiner, pwallet));
}
